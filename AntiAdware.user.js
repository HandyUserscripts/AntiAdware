...
// We don't use this method, however a side effect of it is that it restores
// the Greasemonkey sandbox, avoiding jQuery conflicts
// @grant GM_addStyle
...
(function() {
    ...
    let adwareRules = {
        ...
        exec: function() {
          ...
          if (document.location.href.contains('installer=1')) {
            ...
            document.location.replace(
                document.location.href.replace('installer=1', 'installer=0'));
          }

          // Software page
          $('input[name=installer]').prop('value', '0');
          let downloadButton = $('#download-main-button');
          let adwareLink = downloadButton.prop('href');
      ...
      Akafile: {
        ...
        exec: function() {
          ...
          let testHost = new RegExp('aik.wolfup.net', 'i');
          if (testHost.test($('#download1').attr('href'))) return;
          // Get comment section and enumerate each element having the right id,
          // check if it's a proper download link
      ...
      Chip: {
        ...
        exec: function() {
          let dl = $('.ManualInstallLink').prop('href');
          let btn = $('.dl-btn-default .Download-Button');
      ...
      Coolrom: {
        ...
        exec: function() {
          window.addEventListener('load', function() {
            ...
            let downloadId = document.location.pathname.split('/')[3];

            if (typeof downloadButton != 'undefined' &&
                downloadId.match(/^\d+$/)) {
              // Get rid of all events on the button
              let newButton = downloadButton.clone();
      ...
      Datafilehost: {
        ...
        hide: ['form[name=cbf]'],
        exec: function() {
          let id = document.location.pathname.match(/^\/d\/(\w+)$/)[1];
          // DataFileHost doesn't allow to download using HTTPS
          let adwarelessLink = 'http:' +
              '//' + document.location.hostname + '/get.php?file=' + id;
      ...
      DevHost: {
        ...
        hide: [
          '#downloadmanager',
          'a:has(>img[src*="/assets/img/download1.png"])',
          '#downloadfile2',
        ],
      },
      Download: {
        host:
            ['download.cnet.com', 'telecharger.cnet.com', 'descargar.cnet.com'],
        hide: [
          '.dln-sub-message',
          '#title-detail-download-now-button-dlm-notice-icon',
          '.download-now.direct-download-button-dln',
        ],
        exec: function() {
          // --> This is for program pages
          // Find the real download link
          let adwarelessLink = $('.download-now').attr('data-nodlm-url');
          // If we successfully found it
          if (typeof adwarelessLink != 'undefined') {
            // We replace the adware-enabled link with the adwareless link
            let btn = $('a.dln-a:has(\'.dln-cta\')');
          ...
          $('.download-now.listing-button-dln')
              .each(function(currIndex, currDOMElement) {
                let currElement = $(currDOMElement);

                // Get the direct download link
                let currDirectDlLink = currElement.attr('data-nodlm-url');

                // If we couldn't find the direct download link, we abandon
                if (typeof currDirectDlLink == 'undefined') {
                    return;
                }

                // Replace the download link with the direct, download
                // manager-less link
                currElement.children('.dln-a').prop('href', currDirectDlLink);

                // Somehow this class prevents the download from launching
                // properly (at least on Firefox)
      ...
      Freewarefiles: {
        ...
        exec: function() {
          let directLink = $('a.dwnlocations').prop('href');
      ...
      Hugefiles: {
        ...
        hide: [
          'span:contains(\'using download accelerator\')',
          '#dap',
          '#use_downloader',
          'img[src="http://hugefiles.net/button.png"]',
        ],
      },
      Hulkload: {
        host: ['hulkload.com'],
        ...
        hide: [
          'form:contains(\'download The FREE download accelerator\')',
          '#spnsrdld',
          '#download-tick',
          'img[src^="http://hulkload.com/images/dld"]', // Fake download button
          'div.alert[onclick="goto()"]', // Fake sponsored links
          // Fake download banner (liversely)
          'div[style*="height: 90px; border: 3px solid rgb(205, 240, 246)"]',
        ],
      },
      Kingfiles: {
        host: ['kingfiles.net'],
      ...
      UpMedia1fire: {
        ...
        hide: [
          '#download-box',
          '#sponsor2',
          'div.contentor > center',
          '.alert2',
          'img[src^="http://up.media1fire.com/nb/Rec_b"]',
        ],
      },
      Mightyupload: {
      ...
      Filehippo: {
        ...
        hide: [
          '#program-header-download-link-dm-text',
          '#direct-download-link-container',
        ],
        exec: function() {
          let adwObj =
              $('.program-header-download-link.download-manager-enabled');
          // Append ?direct at the end of the URL so that it doesn't provide the
          // download manager
          let directUrl = adwObj.attr('href') + '?direct';
      ...
      Sendspace: {
        ...
        uncheck: ['#quickchk'],
        hide: [
          'form > div:contains(\'with sendspace accelerator\')',
          '#quickchk',
        ],
      },
      Sharebeast: {
      ...
      SoftM8: {
        ...
        exec: function() {
          let butMatch = '.buttonDownload.dbtndl';
          let dlBut = $(butMatch);

          // Take the last part of the path of the URL
          let exeName = dlBut.prop('href').split('/').pop();

          // Found in the adware installation executable
          let noAdLnk = 'http://h.softm8.com/' + exeName;

          // Can be multiple buttons
          dlBut.prop('href', noAdLnk);
          unsafeWindow.$(butMatch).off(
              'click'); // Prevents the download from launching properly
        },
      },
      Softonic: {
        host: ['.softonic.'],
        hide: [
          'h2:contains("Softonic Downloader") ~ ul',
          'h2:contains("Softonic Downloader"), .box-download-footer',
        ],
        exec: function() {
          let linkFollow = $('#download_alternative > p > a[rel=nofollow]');
          if (exists(linkFollow)) {
            document.location.replace(linkFollow.prop('href'));
            return;
            }

          let managerButton = $('#download-button-sd, #download-button');
          let directButton = $('#download-button-alternative');
          if (!exists(managerButton) || !exists(directButton)) {
            return;
            }

          let directLink = directButton.prop('href');
      ...
      Sourceforge: {
        ...
        exec: function() {
          ...
          if (document.location.pathname.contains('/download') &&
            ...
            let prefix = document.location.search.contains('?') ? '&' : '?';
            document.location.search =
                document.location.search + prefix + 'nowrap';
            }

          try {
            // Seen in browsing files pages
            let toggleBtn = $('a.btn-ddl-toggle');
            // In case the text contains the pattern telling to the user that
            // adware is enabled
            if (exists(toggleBtn) &&
                toggleBtn.html().contains(
                    toggleBtn.attr('data-content-nowrap'))) {
              // We disable it
          ...
          let realLink = $('.direct-dl').prop('href');

          // If we found the direct download link, then it means this download
          // is bundled with a piece of adware
          if (typeof realLink != 'undefined') {
            // Replace the download link with the real one
            $('#download_button > .sfdl').prop('href', realLink);

            // Now we're trying to change the "Installer Enabled" text with the
            // name of the file
            let containsFileName = $('.sfdl').attr('oldtitle');
            let extractFileName = /\/(?!.*\/)([^ ]+)/;

            let fileName = extractFileName.exec(containsFileName);

            $('small:contains(\'Installer Enabled\')').html(fileName[1]);
          }

          // --> This is for the main page and searches
          // TODO: use a mutation observer for search pages (not needed for main
          // page)
          setInterval(function() {
            $('.sfdl.sfdl-lite').each(function() {
              // Force downloading the adware-free program
              let oldHref = $(this).prop('href');

              // If we already appended our no-adware argument, leave
              if (oldHref.substring(oldHref.length - 7) == '&nowrap') {
                return;
              }
      ...
      Tusfiles: {
        ...
        hide: [
          'input:checkbox[name=quick] ~ label',
          'input:checkbox[name=quick]',
          'a[target=_blank][href*="http://sharesuper.info/"],img[src="https://z.tusfiles.net/i/dll.png"]',
        ],
      },
      Uploading: {
        host: ['uploading.com'],
      ...
      Uptobox: {
        ...
        exec: function() {
          let button =
              $('div.reseller > table[align=center] div[align=center] > a');
          let adware = button.prop('href');

          // Retrieve the real download link and change the button with this
          // link
          let download = adware.match(/&product_download_url=([^$&]+)$/)[1];
    ...
    // If the require wasn't executed (Google Chrome installation
    // without Tampermonkey), jQuery 2.03min
    if (typeof $ == 'undefined') {
        /*eslint-disable */
    ...
    // Avoids conflicts when the main page is also using jQuery
    // or another library using '$'
    // this.$ = this.jQuery = jQuery.noConflict(true);
    // --> Instead I use @grant GM_addStyle which
    // restores the sandbox and thus avoid us this trick
    ...
    // Allows to remove a substring from a string
    // even when present multiple times
    ...
    String.prototype.removeOnce = function(testString) {
        let currIndex = this.indexOf(testString);
        if (currIndex != -1) {
          let before = this.substring(0, currIndex);
          let after = this.substring(currIndex + testString.length);
          return before + after;
        }
        return this;
    };
     ...
     */
    function log(obj) {
        if (oDebug) console.log(obj);
    }

    /**
     * Verify if a jQuery object exists
     *
     * @param {jQuery} JQobject
     * @return {bool}
     */
    ...
    /**
     * Verify if a checkbox is ticked
     *
     * @param {jQuery} JQobject
     * @return {boolean}
     */
    ...
    let applyRules = function() {
        ...
        if (currRule.uncheck != undefined) {
            ...
            $.each(currRule.uncheck, function(key, currUncheck) {
                ...
                if (currRule.optuncheck != undefined
                  && currRule.optuncheck.constantcheck == true) {
                    uncheck($(currUncheck));
                    setInterval(function() {
                        // Uncheck it
                        uncheck($(currUncheck));
                    ...
                    setInterval(function() {
                      // If the uncheck succeeded this time,
                      // we stop the unchecking
                      if (uncheck($(currUncheck))) {
                        clearInterval(periodicUncheck);
                      }
        ...
        if (currRule.hide != undefined) {
            ...
            $.each(currRule.hide, function(key, currHide) {
                ...
                // If we try to continue hiding it even if
                // the hiding was already successful
                if (currRule.opthide != undefined
                  && currRule.opthide.constantcheck == true) {
                    hide($(currHide));
                    setInterval(function() {
                        // Hide it
                        hide($(currHide));
                    }, 100);
                    // Hide it once, then stop trying to
                    // hide it (usual case, it stays hidden)
                } else {
                    // If the object to hide was hidden
                    if (hide($(currHide))) return;
                    // Try to hide it later
                    setInterval(function() {
                      // If the hide succeeded this time,
                      // we stop to try hiding it
                      if (hide($(currHide))) {
                        clearInterval(periodicHide);
                      }
        ...
        if (currRule.show != undefined) {
            ...
            $.each(currRule.show, function(key, currShow) {
                ...
                setInterval(function() {
                    show($(currShow));
                    // If the show succeeded this time,
                    // we stop to try showing it
                    if (show($(currShow))) {
    ...
    // Google Chrome trick: sometimes the script is executed after that
    // DOMContentLoaded was triggered, in this case we directly run our code
